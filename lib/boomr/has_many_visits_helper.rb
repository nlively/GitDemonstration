module Boomr::HasManyVisitsHelper
  ...
  def unbatched_visits_by_date_range start, stop
    unbatched_visits.where 'payroll_line_item_id IS NULL AND billable = ? AND in_time BETWEEN ? AND ?', true, start, stop
  end
  ...
  def unbilled_visits_by_date_range start, stop
    unbilled_visits.where 'client_invoice_line_item_id IS NULL AND billable = ? AND in_time BETWEEN ? AND ?', true, start, stop
  end
